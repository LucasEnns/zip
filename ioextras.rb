module ThisEnnsHere
  ...
  # ** encapsulated IOExtras in my namespace
  module IOExtras #:nodoc:
    ...
    module AbstractInputStream
      ...
      def gets(aSepString = $/)
        ...
        bufferIndex = 0
        while (
                (matchIndex = @outputBuffer.index(aSepString, bufferIndex)) ==
                  nil
              )
          bufferIndex = @outputBuffer.length
          return @outputBuffer.empty? ? nil : flush if input_finished?
      ...
      def each_line(aSepString = $/)
        yield readline(aSepString) while true
      rescue EOFError
      end
