    ...
    module ZipFileSystem
      ...
      class ZipFsFile
        ...
        class ZipFsStat
          ...
          forward_message :forward_invoke,
                          :file?,
                          :directory?,
                          :pipe?,
                          :chardev?
        ...
        def open(fileName, openMode = 'r', &block)
          ...
          when 'w'
            ...
            unless openMode == 'r'
              raise StandardError, "openmode '#{openMode} not supported"
            end
          end
        end
        ...
        def utime(modifiedTime, *fileNames)
          fileNames.each { |fileName| get_entry(fileName).time = modifiedTime }
        end
        ...
        def readlink(fileName)
          raise NotImplementedError,
                'The readlink() function is not implemented'
        end
        ...
        def stat(fileName)
          raise Errno::ENOENT, fileName if !exists?(fileName)
      ...
      class ZipFileNameMapper # :nodoc:all
        ...
        def each
          @zipFile.each { |e| yield('/' + e.to_s.chomp('/')) }
        end

        def expand_path(aPath)
          expanded =
            aPath.starts_with('/') ? aPath : @pwd.ensure_end('/') + aPath
          expanded.gsub!(%r{/\.(/|$)}, '')
          expanded.gsub!(%r{[^/]+/\.\.(/|$)}, '')
