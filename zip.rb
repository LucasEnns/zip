...
module ThisEnnsHere
  ...
  # ** unused Zlib constant definition
  module Zip
    using ThisEnnsHere::MonkeyPatches
    ...
    class ZipInputStream
      ...
      def initialize(filename, offset = 0)
        super()
        @archiveIO =
          filename.class == StringIO ? filename : File.open(filename, 'rb')
      ...
      def get_next_entry
        if @currentEntry
          @archiveIO.seek(@currentEntry.next_header_offset, IO::SEEK_SET)
        end
        open_entry
      end
      ...
      def open_entry
        ...
        if (@currentEntry == nil)
          ...
          @decompressor =
            PassThruDecompressor.new(@archiveIO, @currentEntry.size)
    ...
    class Inflater < Decompressor #:nodoc:all
      ...
      def sysread(numberOfBytes = nil, buf = nil)
        ...
        if @outputBuffer.length == 0 && input_finished?
          return value_when_finished
        end
      ...
      def internal_produce_input(buf = nil)
        ...
        begin
          s =
            (
              if buf
                @inputStream.read(Decompressor::CHUNK_SIZE, buf)
              else
                @inputStream.read(Decompressor::CHUNK_SIZE)
              end
            )
    ...
    class ZipEntry
      ...
      def initialize(
        ...
        compressed_size = 0,
        crc = 0,
        compression_method = ZipEntry::DEFLATED,
        size = 0,
        time = Time.now
      )
        super()
        if name.starts_with('/')
          raise ZipEntryNameError,
                "Illegal ZipEntry name '#{name}', name must not start with /"
        end
        @localHeaderOffset = 0
        ...
        @extra = ZipExtraField.new(@extra.to_s) unless ZipExtraField === @extra
      end
      ...
      def time=(aTime)
        @extra.create('UniversalTime') unless @extra.member?('UniversalTime')
      ...
      def directory?
        unless @ftype
          raise ZipInternalError, "current filetype is unknown: #{self.inspect}"
        end
        @ftype == :directory
      end
      ...
      def local_header_size #:nodoc:all
        LOCAL_ENTRY_STATIC_HEADER_LENGTH + (@name ? @name.size : 0) +
      ...
      def read_local_entry(io) #:nodoc:all
        ...
        unless (staticSizedFieldsBuf.size == LOCAL_ENTRY_STATIC_HEADER_LENGTH)
          raise ZipError,
                'Premature end of file. Not enough data for zip entry local header'
        end
        ...
        nameLength,
        extraLength =
          staticSizedFieldsBuf.unpack('VCCvvvvVVVvv')

        unless (@header_signature == LOCAL_ENTRY_SIGNATURE)
          raise ZipError,
                "Zip local header magic not found at location '#{localHeaderOffset}'"
        end
      ...
      def read_c_dir_entry(io) #:nodoc:all
        ...
        unless (staticSizedFieldsBuf.size == CDIR_ENTRY_STATIC_HEADER_LENGTH)
          raise ZipError,
                'Premature end of file. Not enough data for zip cdir entry header'
        end
        ...
        @extra,
        @comment =
          staticSizedFieldsBuf.unpack('VCCvvvvvVVVvvvvvVV')
        ...
        when FSTYPE_UNIX
          ...
          when 012
            ...
            raise ZipInternalError,
                  "unknown file type #{'0%o' % (@externalFileAttributes >> 28)}"
          end
        else
          if name_is_directory?
            @ftype = :directory
      ...
      def set_extra_attributes_on_path(destPath)
        ...
        when FSTYPE_UNIX
          ...
          if (@restore_permissions && @unix_perms)
            File.chmod(@unix_perms & unix_perms_mask, destPath)
          end
          if (@restore_ownership && @unix_uid && @unix_gid && Process.egid == 0)
            File.chown(@unix_uid, @unix_gid, destPath)
          end
          # File::utimes()
        end
      end
      ...
      def ==(other)
        ...
        (
          @compression_method == other.compression_method &&
            @crc == other.crc && @compressed_size == other.compressed_size &&
            @size == other.size && @name == other.name &&
            @extra == other.extra && @filepath == other.filepath &&
      ...
      def gather_fileinfo_from_srcpath(srcPath) # :nodoc:
        ...
        when 'link'
          ...
          raise RuntimeError,
                "unknown file type: #{srcPath.inspect} #{stat.inspect}"
        end
      ...
      include ThisEnnsHere::MonkeyPatches
    ...
    class ZipOutputStream
      ...
      def put_next_entry(entry, level = Zlib::DEFAULT_COMPRESSION)
        ...
        newEntry =
          entry.kind_of?(ZipEntry) ? entry : ZipEntry.new(@fileName, entry.to_s)
      ...
      def copy_raw_entry(entry)
        ...
        @compressor = NullCompressor.instance
        @outputStream << entry.get_raw_input_stream do |is|
      ...
      def finalize_current_entry
        ...
        @currentEntry.compressed_size =
          @outputStream.tell - @currentEntry.localHeaderOffset -
      ...
      def get_compressor(entry, level)
        ...
        when ZipEntry::DEFLATED
          Deflater.new(@outputStream, level)
        when ZipEntry::STORED
          PassThruCompressor.new(@outputStream)
        else
          raise ZipCompressionMethodError,
    ...
    class Deflater < Compressor #:nodoc:all
      ...
      def finish
        @outputStream << @zlibDeflater.finish until @zlibDeflater.finished?
      end
    ...
    class ZipCentralDirectory
      ...
      MAX_END_OF_CENTRAL_DIRECTORY_STRUCTURE_SIZE = 65_536 + 18
      STATIC_EOCD_SIZE = 22
      ...
      def initialize(entries = ZipEntrySet.new, comment = '') #:nodoc:
        super()
        @entrySet =
          entries.kind_of?(ZipEntrySet) ? entries : ZipEntrySet.new(entries)
        @comment = comment
      end
      ...
      def read_e_o_c_d(io) #:nodoc:
        ...
        unless buf.size == 0
          raise ZipError, 'Zip consistency problem while reading eocd structure'
        end
      end
      ...
      def read_central_directory_entries(io) #:nodoc:
        begin
          ...
          raise ZipError,
                'Zip consistency problem while reading central directory entry'
        end
        @entrySet = ZipEntrySet.new
        @size.times { @entrySet << ZipEntry.read_c_dir_entry(io) }
      end
      ...
      def get_e_o_c_d(io) #:nodoc:
        ...
        unless sigIndex
          raise ZipError, 'Zip end of central directory signature not found'
        end
    ...
    class ZipFile < ZipCentralDirectory
      ...
      def get_output_stream(entry, &aProc)
        newEntry =
          entry.kind_of?(ZipEntry) ? entry : ZipEntry.new(@name, entry.to_s)
      ...
      def get_entry(entry)
        ...
        raise Errno::ENOENT, entry unless selectedEntry
      ...
      def on_success_replace(aFilename)
        ...
        tmpfile.close
        FileUtils.mv(tmpFilename, name) if yield tmpFilename
      end
    ...
    class ZipStreamableStream < DelegateClass(ZipEntry) #nodoc:all
      ...
      def get_input_stream
        if !@tempFile.closed?
          raise StandardError,
                "cannot open entry for reading while its open for writing - #{name}"
        end
    ...
    class ZipExtraField < Hash
      ...
      class Generic
        ...
        def ==(other)
          ...
          each { |k, v| v != other[k] and return false }
          true
        end
      ...
      class UniversalTime < Generic
        ...
        def ==(other)
          @mtime == other.mtime && @atime == other.atime &&
            @ctime == other.ctime
        end
      ...
      def create(name)
        field_class = nil
        ID_MAP.each do |id, klass|
          if klass.name == name
            field_class = klass
            break
          end
        end
        raise ZipError, "Unknown extra field '#{name}'" if !field_class
